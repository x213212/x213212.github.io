...
Controller
Controller 用來定義使用者介面對使用者輸入的回應方式，它連線 Model 和 View，用於控制應用程式的流程，處理使用者的行為和資料上的改變。當用戶對 View 進行操作時，在 Controller 中回應 View 的事件，並呼叫 Model 的介面對資料進行操作，一旦 Model 發生變化便通知相關檢視進行更新。  
