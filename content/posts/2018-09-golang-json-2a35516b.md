...
這樣跳來跳去進度會不會超太快xd 剛剛的教學已經大致上完成一部分了，大家比較有問題的在 要改直接改原始碼的話，就注意資料庫的ip和密碼和table表， 還有在route的網址所對應的網址基本上應該是沒什麼難度 golang的時間轉timestamp
...
伺服器方面的部分呢還是用ngork進行對外連接，今天差不多就到這裡，golang速度超快，其實做到這邊，golang就差不多熟了，那可能比較深入一些函式像是執行續拉等等沒碰到，golang應該很快就可以上手。  
